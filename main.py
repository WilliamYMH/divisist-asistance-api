...
def iniciar_sesion(session, payload):
    try:
        session.post("https://divisist2.ufps.edu.co", data=payload, verify=False)
...
def get_notas_parciales(session):
    ...
    s_page = session.get('https://divisist2.ufps.edu.co/informacion_academica/materias')
        ...
        for i in col:
            ...
            if(nombre and nombre[0]!='' and nombre[0].contents):
                if(str(nombre[0].contents[0]).strip()!='×'):
                    notas[map_eq.get(contador)]=str(nombre[0].contents[0]).strip()
                    contador+=1
...
def get_nombres_materias(session):
    try:
        ...
        for row in table.findAll('tr'):
            col = row.findAll('td')
            for i in col:
                nombre=i.findAll('h4', {"class":"modal-title", "id":"myModalLabel"})
